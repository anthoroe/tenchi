...
function TenchiEngine(client) {
  ...
  this.login_server = new TenchiNetwork(this.events);
  this.world_server = new TenchiNetwork(this.events);
  ...
  // login event
  this.events.on('login_success', function(message) {
    self.world_server.init(message.s);
  });
  // player info
